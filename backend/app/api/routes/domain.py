...
from app.schemas.domain import AssignmentCreate, AssignmentUpdate, ClassCreate, InterestsRequest, JoinClassRequest
...
def class_data(item: Class) -> dict:
    return {"id": item.id, "teacher_id": item.teacher_id, "name": item.name, "subject": item.subject, "grade_level": item.grade_level, "description": item.description, "join_code": item.join_code, "created_at": item.created_at}
...
def create_class(data: ClassCreate, db: Annotated[Session, Depends(get_db)], teacher: Annotated[Profile, Depends(get_teacher)]):
    return class_data(service.create_class(db, teacher, data))
...
def list_classes(db: Annotated[Session, Depends(get_db)], profile: Annotated[Profile, Depends(get_authenticated_profile)]):
    ...
    return {"items": [class_data(item) for item in items], "total": len(items)}
...
def get_class(class_id: uuid.UUID, db: Annotated[Session, Depends(get_db)], profile: Annotated[Profile, Depends(get_authenticated_profile)]):
    return class_data(service.class_for_user(db, class_id, profile))
