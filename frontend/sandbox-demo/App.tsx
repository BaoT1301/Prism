...
export function App() {
  ...
  return <div className="demo-shell" data-testid="sandbox-demo"><header className="demo-header"><div><h1 className="product-title">PRISM INTERACTIVE SANDBOX</h1></div><div className="demo-controls"><label>Fixture<select value={fixtureName} data-testid="fixture-select" onChange={(event) => setFixtureName(event.target.value as FixtureName)}><option value="basketball">Basketball</option><option value="formula1">Formula 1</option><option value="space">Space</option></select></label><button type="button" data-testid="reset-demo" onClick={resetDemo}>Reset demo</button></div></header><section className="demo-status">{launch && <span data-testid="session-version">Session version: {launch.session.version}</span>}<span>Progress is saved in this browser.</span></section>{loading && <p className="center-message" role="status">Loading sandbox...</p>}{error && <p className="center-message error" role="alert">{error}</p>}{launch && <SandboxRenderer key={`${fixtureName}-${reloadKey}`} spec={launch.assignment.sandbox_spec} session={launch.session} api={api} />}</div>;
}
