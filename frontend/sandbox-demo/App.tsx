...
export function App() {
  ...
  return <div className="demo-shell"><header className="demo-header"><div><p className="eyebrow">Prism interactive sandbox</p><h1>Sandbox browser demo</h1><p>Local demo mode uses the same renderer contract and interaction flow without requiring authentication.</p></div><div className="demo-controls"><label>Fixture<select value={fixtureName} onChange={(event) => setFixtureName(event.target.value as FixtureName)}><option value="basketball">Basketball</option><option value="formula1">Formula 1</option><option value="space">Space</option></select></label><button type="button" onClick={resetDemo}>Reset demo</button></div></header><section className="demo-status"><span>Mode: local demo API</span>{launch && <span>Session version: {launch.session.version}</span>}<span>Progress is saved in this browser.</span></section>{loading && <p className="center-message">Loading sandbox…</p>}{error && <p className="center-message error" role="alert">{error}</p>}{launch && <SandboxRenderer key={`${fixtureName}-${reloadKey}`} spec={launch.assignment.sandbox_spec} session={launch.session} api={api} />}</div>;
}
