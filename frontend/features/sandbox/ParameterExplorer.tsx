...
export function ParameterExplorer({ spec, initialSession, api, onSubmitted }: { spec: SandboxSpec; initialSession: SandboxSession; api: SandboxApi; onSubmitted?: () => void }) {
  ...
  useEffect(() => { if (satisfiedAutomaticIds.length === 0) return; setCompletedStepIds((current) => { const next = mergeCompletedStepIds(spec, current, values, reflectionAnswers); return next.length === current.length ? current : next; }); }, [reflectionAnswers, satisfiedAutomaticIds, spec, values]);
  useEffect(() => { if (firstRender.current) { firstRender.current = false; return; } const timer = window.setTimeout(async () => { setSaveStatus("saving"); try { const latest = await api.updateProgress(session.id, buildProgressRequest(session, completedStepIds, values)); setSession((current) => ({ ...current, ...latest })); setSaveStatus("saved"); } catch (error) { if (error instanceof SandboxApiError && error.status === 409) { const latest = await api.getSession(session.id); setSession(latest); setCompletedStepIds(latest.completed_step_ids); setValues((current) => ({ ...current, ...latest.responses })); setSaveStatus("conflict"); } else setSaveStatus("error"); } }, 400); return () => window.clearTimeout(timer); }, [api, completedStepIds, session, values]);
  ...
  return <main className={`sandbox-main theme-${spec.visual_theme ?? "basketball"}`}>
    ...
    <div className="sandbox-dashboard"><div className="simulation-column"><PhysicsScene spec={spec} values={values} runToken={runToken} /><div className="simulation-action"><div><p className="card-kicker">Ready when you are</p><strong>Change a variable, then run your experiment.</strong></div><button className="primary-button" type="button" onClick={() => { setRunToken((token) => token + 1); setCompletedStepIds((current) => current.includes("run-simulation") ? current : [...current, "run-simulation"]); }}>▶ Run Simulation</button></div></div><aside className="coach-column"><HintPanel hint={hint} remaining={hint?.remaining_hint_levels ?? Math.max(0, 3 - session.hints_used)} onRequest={requestHint} />{hintError && <p className="inline-error" role="alert">{hintError}</p>}<div className="objective-card"><p className="card-kicker">Learning objective</p><p>{spec.introduction}</p><span className="objective-tag">F = ma</span></div></aside></div>
    ...
    <div className="completion-bar"><SaveStatus status={saveStatus} /><div><p>{missionComplete ? "You have completed every experiment step." : "Finish the checklist and reflection to complete your mission."}</p><button className="complete-button" type="button" disabled={submitting || session.status === "submitted" || !missionComplete} onClick={() => void submit()}>{session.status === "submitted" ? "✓ Mission Complete" : submitting ? "Saving mission..." : "Complete Mission →"}</button></div></div>
  </main>;
}
